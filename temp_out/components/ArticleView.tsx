...
const ArticleView: React.FC<ArticleViewProps> = ({ article, onBack, onArticleSelect, allArticles, onShowMethodology }) => {
  ...
  return (
    <div className="bg-zinc-950 min-h-screen animate-fade-in text-white">
      ...
      <div className="container mx-auto px-4 md:px-12 pt-24 md:pt-32 pb-24 max-w-4xl">
        <button 
          onClick={onBack}
          className="group flex items-center gap-2 text-gray-400 hover:text-white transition-colors mb-6 md:mb-8 font-medium text-xs tracking-widest uppercase"
        >
        ...
        <header className="mb-8 md:mb-10 text-left">
          <div className="inline-block text-news-accent font-bold uppercase tracking-widest text-xs mb-4 md:mb-6 border-b border-news-accent pb-1">
        ...
        <figure className="mb-12 md:mb-16 -mx-4 md:-mx-12 lg:mx-0">
          ...
          <figcaption className="text-left text-[10px] uppercase tracking-widest text-gray-500 mt-3 px-4 md:px-0">
            Image: Unsplash Licensed / Illustration
          </figcaption>
        </figure>
        ...
        <div className="prose prose-lg md:prose-xl prose-invert max-w-none md:px-8 font-sans leading-relaxed text-gray-200">
          {article.content?.map((paragraph, index) => {
            ...
            if (isSubheader) {
              return <h3 key={index} className="text-2xl md:text-3xl font-serif font-bold text-white mt-12 mb-6 border-l-4 border-news-accent pl-4">{paragraph}</h3>
            }

            const contentElement = (
              <p key={index} className={`mb-8 ${index === 0 ? 'first-letter:text-6xl first-letter:font-serif first-letter:font-bold first-letter:float-left first-letter:mr-3 first-letter:mt-[-8px] first-letter:text-white' : ''}`}>
                {paragraph}
              </p>
            );

            // Integrated Quote after first paragraph
            if (index === 0) {
              return (
                <React.Fragment key={index}>
                  {contentElement}
                  <div className="my-10 md:my-14 pl-4 md:pl-8 border-l-2 border-news-accent">
            ...
            // Context Box after second paragraph
            if (index === 1) {
              return (
                <React.Fragment key={index}>
                  {contentElement}
            ...
            if (index === 4) {
                return (
                    <React.Fragment key={index}>
                        {contentElement}
                        <AdUnit className="w-full h-64 my-12" format="rectangle" />
                    </React.Fragment>
                )
            }
        ...
        <div className="mt-16 mb-8">
            <AdUnit className="w-full h-32 md:h-48" format="horizontal" />
        </div>
        ...
        {relatedArticles.length > 0 && (
          <div className="mt-12 mb-12 border-t border-white/10 pt-16">
        ...
        <div className="mt-12 pt-10 border-t border-white/10 flex flex-col items-center text-center">
           <p className="text-gray-400 italic mb-6">
             You have reached the end of this report.
           </p>
           <button 
             onClick={onBack}
